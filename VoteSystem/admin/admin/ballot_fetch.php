<?php
	...
	while($row = $query->fetch_assoc()){
		...
		while($crow = $cquery->fetch_assoc()){
			...
			$candidate .= '
				<li>
					'.$input.'<button class="btn btn-primary btn-sm btn-flat clist"><i class="fa fa-search"></i> Poll detail</button><img src="'.$image.'" height="100px" width="100px" class="clist"><span class="cname clist">'.$crow['firstname'].' '.$crow['lastname'].'</span>
				</li>
			';
		}

		$instruct = ($row['max_vote'] > 1) ? 'Maximum Vote '.$row['max_vote'].' ' : 'Select only one candidate';
